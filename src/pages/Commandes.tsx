...
export const Commandes = () => {
  ...
  const [stats, setStats] = useState({ totales: 0, en_attente: 0, en_production: 0, livrees: 0 });
 ...
 useEffect(() => {
  loadData();
}, [filterStatus, filterInstitution, filterPriority, searchTerm, dateDebut, dateFin, currentPage]);
...
const loadData = async () => {
  try {
    ...
    // CORRECTION: Accéder correctement aux données de stats
    // L'API retourne: { data: { total_commandes, par_statut: [...], montant_livrees, ... } }
    ...
    const statsMap: any = {
      ...
      en_production: 0,
      livrees: 0,
      montant_livrees: 0  // Ajouter le montant des commandes livrées
    };
    ...
    statsMap.livrees = statsData.nombre_livrees || 0;
    statsMap.montant_livrees = statsData.montant_livrees || 0;
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const response = await pdfService.generateReport('commandes', commandes, filters);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {commandes.map((commande) => (
                    <tr key={commande.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-military-700">{commande.numero}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{formatCurrency(commande.montant)}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <StatusBadge
                          currentStatus={commande.statut}
                          availableStatuses={['En attente', 'En production', 'Livrée']}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{formatDate(commande.date_commande)}</td>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-base font-semibold text-gray-900">{formatCurrency(viewingCommande.montant)}</p>
              </div>
              <div>
